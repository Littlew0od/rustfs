...
impl S3Client {
    pub async fn new(config: &Config) -> Result<Self> {
        ...
        let credentials = Credentials::new(access_key, secret_key, None, None, "rustfs-tftp-server");
    ...
    pub async fn head_bucket(&self, bucket_name: &str) -> Result<()> {
        ...
        self.client
            ...
            .send()
            .await
            .context(format!("Failed to head bucket: {}", bucket_name))?;

        Ok(())
    }

    pub async fn upload_file(&self, local_path: &str, bucket_name: &str, object_key: &str) -> Result<UploadResult> {
        ...
        // 不确定对不对
        let content_type = mime_guess::from_path(&canonical_path).first_or_octet_stream().to_string();
        debug!("Auto-detected content type: {content_type}");
    ...
    pub async fn download_object_to_file(&self, bucket_name: &str, object_key: &str, local_path: &str) -> Result<(u64, String)> {
        info!("Downloading object: s3://{}/{} -> {}", bucket_name, object_key, local_path);

        let request = self.client.get_object().bucket(bucket_name).key(object_key);
    ...
    pub async fn health_check(&self) -> Result<()> {
        ...
        self.client.list_buckets().max_buckets(1).send().await.context("S3 health check failed")?;
